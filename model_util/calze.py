...
##ze[mask] += 7.295e13*(rho[mask]**2.)*(qr[mask]**2.0)/nr[mask]
#ze[mask] += 7.295e13*(rho[mask]**2.)*(qr[mask]**2.0)/nr[mask]
ze[mask] += 7.295e13*((qr[mask]*rho[mask])**2.0)/nr[mask]

qi = data["QI"].data*1e-3 # same
ni = data["NI"].data*1e3

mask = (qi > qmin) & (ni > nmin)
##ze[mask] += 4.4327*(rho[mask]**3.)*(qi[mask]**3.0)/(ni[mask]**2.0)
#ze[mask] += 508543.e18*(rho[mask]**3.)*(qi[mask]**3.0)/(ni[mask]**2.0)
ze[mask] += 508543.e18*((qi[mask]*rho[mask])**3.0)/(ni[mask]**2.)

qg = data["QG"].data*1e-3 # same
ng = data["NG"].data*1e3

mask = (qg > qmin) & (ng > nmin)
##ze[mask] += 8.95845e14*(rho[mask]**2.)*(qg[mask]**2.0)/ng[mask]
#ze[mask] += 4.73e15*(rho[mask]**2.)*(qg[mask]**2.0)/ng[mask]
ze[mask] += 4.74e15*((qg[mask]*rho[mask])**2.0)/ng[mask]
...
def radar_colormap(simple=False):
    if simple:
...
#cmap = radar_colormap(simple=True)
